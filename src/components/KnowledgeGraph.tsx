...
const Legend = () => (
  <div className="absolute top-4 right-4 bg-gray-100/90 dark:bg-black/50 backdrop-blur-md border border-gray-300 dark:border-white/10 rounded-lg p-4 text-gray-900 dark:text-white">
...
const KnowledgeGraph = () => {
  ...
  const nodeColor = (node: GraphNode) => {
    if (hoveredNode) {
      ...
      return isConnected ? statusColors[node.status] : isDarkMode() ? '#333' : '#9ca3af';
    }
  ...
  const linkColor = (link: GraphLink) => {
    if (hoveredNode) {
      ...
      const inactiveColor = isDarkMode() ? 'rgba(255,255,255,0.2)' : 'rgba(0,0,0,0.2)';
      return isConnected ? (link.type === 'tag' ? 'rgba(139, 92, 246, 0.8)' : 'rgba(139, 92, 246, 0.9)') : inactiveColor;
    }
    ...
    <div className="relative w-full h-screen">
      <ForceGraph2D
        ...
        nodeCanvasObject={(node: GraphNode, ctx: CanvasRenderingContext2D, globalScale: number) => {
          ...
          ctx.fillStyle = darkMode ? 'rgba(0,0,0,0.8)' : 'rgba(255,255,255,0.95)';
          ctx.fillRect(
            node.x! - bckgWidth / 2,
      ...
      />
      <Legend />
      <div className="absolute bottom-4 left-4 text-gray-900 dark:text-white text-sm bg-gray-100/90 dark:bg-black/50 px-3 py-2 rounded">
